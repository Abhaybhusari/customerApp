...
public class MainActivity extends SlidingFragmentActivity implements OnInitializedListener {

	private Fragment m_Fragment;
	private ArrayList<DepartInfo> aryDepartList = new ArrayList<DepartInfo>();
	private DisplayImageOptions options, optionsForBg, optionsMenuHouse, optionsMenuSpa, optionsMenuNews, optionsMenuTour;
...
//		String strMsg = String.format("isTab = " + DeviceUtil.isTabletByRes(this) + 
...
//		MessageBox.OK(this, "", strMsg);
	...
	public void onBackPressed() {
		...
		if(aryFragList.size() == 0 || 
				(m_Fragment != null && 
				m_Fragment.getClass().getName() == HomeFragment.class.getName())) {
	...
	private void updateLCD() {
		...
		optionsMenuHouse = new DisplayImageOptions.Builder()
			...
			.showImageOnFail(R.drawable.housekeeping)
			.cacheInMemory(false)
			.cacheOnDisc(true)
			.bitmapConfig(Bitmap.Config.RGB_565)
			.build();
		...
		optionsMenuSpa = new DisplayImageOptions.Builder()
			...
			.showImageOnFail(R.drawable.massage)
			.cacheInMemory(false)
			.cacheOnDisc(true)
			.bitmapConfig(Bitmap.Config.RGB_565)
			.build();
		...
		optionsMenuNews = new DisplayImageOptions.Builder()
			...
			.showImageOnFail(R.drawable.news)
			.cacheInMemory(false)
			.cacheOnDisc(true)
			.bitmapConfig(Bitmap.Config.RGB_565)
			.build();
		...
		optionsMenuTour = new DisplayImageOptions.Builder()
			...
			.showImageOnFail(R.drawable.tour)
			.cacheInMemory(false)
			.cacheOnDisc(true)
			.bitmapConfig(Bitmap.Config.RGB_565)
			.build();
		...
		btWeather.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				/*if(isDemo) {
					MessageBox.OK(MainActivity.this, "Alert", "Demo version doesn't provide Weather");
					return;
				}*/
		...
		btMsg.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if(isDemo) {
					MessageBox.OK(MainActivity.this, "Alert", "Demo version doesn't provide Message In-Box");
					return;
				}
		...
		btCart.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				/*if(isDemo) {
					MessageBox.OK(MainActivity.this, "Alert", "Demo version doesn't provide Cart");
					return;
				}*/
...
//		}
	...
	{
		if(m_Fragment != null && 
				m_Fragment.getClass().getName() == CartListFragment.class.getName())
			return;
	...
	public void goMsg() {
		...
		if(m_Fragment != null && 
				m_Fragment.getClass().getName() == NotificationFragment.class.getName()) {
	...
	public void goWeather() {
		...
		if(m_Fragment != null && 
				m_Fragment.getClass().getName() == WeatherFragment.class.getName())
			return;
	...
	private void loadDepartList() {
		...
		if (NetworkUtils.haveInternet(this)) {
			...
			CustomerHttpClient.get("Department/GetDepartments.php", params, new AsyncHttpResponseHandler() {
				@Override
				...
				public void onFailure(int statusCode, Header[] headers,	byte[] errorResponse, Throwable e) {
					...
					Toast.makeText(MainActivity.this, "Connection was lost (" + statusCode + ")", Toast.LENGTH_LONG).show();
	            ...
	            public void onSuccess(int statusCode, Header[] headers, byte[] response) {
	            	...
	            	try {
						        ...
						        "depart": [
						            {
						                "id": "1",
						                "title": "House Keeping",
						                "desc": "We are here to make your stay one of the best you have ever had please ask us for anything anytime we are happy to serve",
						                "depimg": "http://www.roomallocator.com/appybiteRestaurant/depimages/houskeeping.jpg"
						            }
						        ],
						        "hotel_image": {
		            	...
		            	*/
	            		...
	            		JSONArray departArray = data.getJSONArray("depart");
	            		
	            		for (int i = 0; i < departArray.length(); i++) {
							
	            			DepartInfo item = new DepartInfo();
	            			
	            			JSONObject object = departArray.getJSONObject(i);
	            		...
	            		JSONObject hotel_image = data.getJSONObject("hotel_image");
	            		PrefValue.setString(MainActivity.this, R.string.pref_hotel_logo, hotel_image.getString("hotel_logo"));
	            		PrefValue.setString(MainActivity.this, R.string.pref_hotel_bg, hotel_image.getString("app_header"));
	            		aryDepartList.get(0).image = hotel_image.getString("app_header");
	            		
	            		if(DeviceUtil.isTabletByRes(MainActivity.this)) {
	            			ImageLoader.getInstance().displayImage(PrefValue.getString(MainActivity.this, R.string.pref_hotel_bg), ivHotelBg, optionsForBg, animateFirstListener);
	            			ImageLoader.getInstance().displayImage(PrefValue.getString(MainActivity.this, R.string.pref_hotel_logo), ivHotelLogo, null, animateFirstListener);
	            			tvHotelName.setText(PrefValue.getString(MainActivity.this, R.string.pref_hotel_name));
	            		} else {
	...
	private void loadRestaurantList() {
		...
		info.title = "Welcome";
		info.desc = "Welcome";
		info.image = PrefValue.getString(MainActivity.this, R.string.pref_hotel_bg);
		aryDepartList.add(info);
						    ...
						    "data": [
						        {
						            ...
						            "name": "court",
						            "description": "",
						            "image": "http://www.roomallocator.com/appcreator/uploads/45.jpg"
						        }
						    ]
						}
		            	*/
	            		...
	            		Log.i("HTTP Response <<<", result);
	            		JSONObject jsonObject = new JSONObject(result);
	            		JSONArray restArray = jsonObject.getJSONArray("data");
	            		
	            		for (int i = 0; i < restArray.length(); i++) {
							
	            			DepartInfo item = new DepartInfo();
	            			
	            			JSONObject object = restArray.getJSONObject(i);
	            			item.id = object.getInt("id");
	...
	private void updateDepartList()
	{
		...
		for (int i = 0; i < aryDepartList.size(); i++) {
			...
			View item = null;
			if(DeviceUtil.isTabletByRes(this))
				item = vi.inflate(R.layout.item_depart_tab, llDepartList, false);
			else
			...
			if (isDemo) {
				if(value.id == 0) {
					...
					ImageLoader.getInstance().displayImage(value.image, ivThumb, optionsMenuHouse, animateFirstListener);
				} else if(value.id == 2) {
					ImageLoader.getInstance().displayImage(value.image, ivThumb, optionsMenuSpa, animateFirstListener);
				} else if(value.id == 3) {
					ImageLoader.getInstance().displayImage(value.image, ivThumb, optionsMenuTour, animateFirstListener);
				} else if(value.id == 97) {
					ImageLoader.getInstance().displayImage(value.image, ivThumb, optionsMenuNews, animateFirstListener);
				} else {
					ImageLoader.getInstance().displayImage(value.image, ivThumb, options, animateFirstListener);
				}
			} else {
				if(value.id == 0) {
					ivThumb.setImageResource(R.drawable.bg_default_restaurant);
	            	...
	            	try {
		            	...
		            	*/
	            		...
	            		if(status.equalsIgnoreCase("true")) {
	            			...
	            			String hotel_id = PrefValue.getString(MainActivity.this, R.string.pref_hotel_id);
	            			if(qrHotelID.equalsIgnoreCase(hotel_id)) {
	            				...
	            				if(order_type.equalsIgnoreCase(ItemDetailsFragment.order_inrestaurant)) {
	            					if(jsonObject.has("tableID"))
	            						...
	            						MessageBox.OK(MainActivity.this, "QRCode Error", "You scanned Room QRCode.\nPlease select correct order type before scan");
	            				} else {

	            					if(jsonObject.has("roomID"))
	            						no = jsonObject.getString("roomID");
	            					else
	            						MessageBox.OK(MainActivity.this, "QRCode Error", "You scanned Table QRCode.\nPlease select correct order type before scan");
	            				}
	            			} else {
	            				
	            				MessageBox.OK(MainActivity.this, "QRCode Error", "You scanned QRCode of different Hotel\nPlease scan QRCode of this hotel");
	            			}
	            		} else {
	            			
	            			MessageBox.OK(MainActivity.this, "QRCode Error", "This QRCode has been expired");
	            		}
		            	...
		            	*/
	    				...
	    				if(array.length() > 0)
	    				{
		    				...
		    				if(m_Fragment != null && 
		    						m_Fragment.getClass().getName() == ItemDetailsFragment.class.getName())
		    				{
		    					PrefValue.setString(MainActivity.this, R.string.pref_order_code, code);
	    				...
	    				MessageBox.OK(MainActivity.this, "QRCode Error", "Error occured while loading QRCode Details");
	...
	public void showMessagePopup() {
		...
		if(cnt > 0) {
			...
			RelativeLayout rlNewMsg = (RelativeLayout)dialog.findViewById(R.id.rlNewMsg);
			...
			TextView tvNewMsgCnt = (TextView)dialog.findViewById(R.id.tvNewMsgCnt);
			...
			if (PRJFUNC.mGrp != null) {
				RelativeLayout rlNewMsgDlg = (RelativeLayout)dialog.findViewById(R.id.rlNewMsgDlg);
				PRJFUNC.mGrp.relayoutView(rlNewMsgDlg, LayoutLib.LP_RelativeLayout);
				PRJFUNC.mGrp.repaddingView(rlNewMsgDlg);
				
				PRJFUNC.mGrp.relayoutView(rlNewMsg, LayoutLib.LP_RelativeLayout);
				
				PRJFUNC.mGrp.relayoutView(ivNewMsg, LayoutLib.LP_RelativeLayout);
				
				PRJFUNC.mGrp.relayoutView(tvNewMsgCnt, LayoutLib.LP_RelativeLayout);
				PRJFUNC.mGrp.setTextViewFontScale(tvNewMsgCnt);
				PRJFUNC.mGrp.repaddingView(tvNewMsgCnt);
				
				PRJFUNC.mGrp.relayoutView(tvNewMsg, LayoutLib.LP_RelativeLayout);
	...
	public void loadWeather() {
		...
		if (NetworkUtils.haveInternet(this)) {
			...
			RequestParams params = new RequestParams();
			params.add("lat", PrefValue.getString(this, R.string.pref_hotel_lat));
			params.add("lon", PrefValue.getString(this, R.string.pref_hotel_lon));
	...
	private void updateWeather(Weather weather) {
		...
		if(weather != null && weather.getData_receiving() != 0) {
			...
			if (weatherId == 801) {
				//. scattered_clouds
				btWeather.setBackgroundResource(R.drawable.weather_partly_cloudy);
			}
			if (weatherId == 802) {
				//. scattered_clouds
				btWeather.setBackgroundResource(R.drawable.weather_partly_cloudy);
...
//						+ curHour + ":" + curMinute + "." + curSecond);
	...
	protected void Update() {
		...
		Runnable updater = new Runnable() {
			public void run() {
//				tvCurrentTime.setText("í˜„ìž¬ ë‚ ì§œì™€ ì‹œê°„ì�€ " + curYear + "ë…„ " + curMonth
...
//						+ curMinute + "ë¶„ " + curSecond + "ì´ˆ ìž…ë‹ˆë‹¤. ");
	...
	public void addYoutubeView(String videoID) {
		...
		if(youTubePlayerFragment != null && 
				youTubePlayerFragment.getClass().getName() == YouTubePlayerSupportFragment.class.getName())
			return;
...
//			
//			DialogUtils.launchProgress(this, "Please wait while checking bookable department");
//			CustomerHttpClient.get("allbookdept.php", params, new AsyncHttpResponseHandler() {
//				
//				@Override
//				public void onFinish() {
//					
...
//	            		} else {
//	            			
//	            			MessageBox.OK(MainActivity.this, "Alert", "Checking bookable department failed");
//	            		}
//	            		
//					} catch (JSONException e) {
//						// TODO Auto-generated catch block
...
//	}
	            	...
	            	try {
		            	...
		            	*/
	            		...
	            		if(status.equalsIgnoreCase("true")) {
		            		...
		            		JSONObject data = jsonObject.getJSONObject("data");
		            		PrefValue.setString(MainActivity.this, R.string.pref_hotel_name, data.getString("hotel"));
		            		PrefValue.setString(MainActivity.this, R.string.pref_hotel_logo, data.getString("logo"));
		            		PrefValue.setString(MainActivity.this, R.string.pref_hotel_bg, data.getString("resturantimg"));
	...
	private void loadDemoDepartments() {
		...
		if (NetworkUtils.haveInternet(this)) {
			...
			CustomerHttpClient.getFromFullService("http://www.roomallocator.com/appcreator/services/getDepartmentItemsAll.php", params, new AsyncHttpResponseHandler() {
				@Override
				public void onFinish() {
					DialogUtils.exitProgress();
					super.onFinish();
	            ...
	            public void onSuccess(int statusCode, Header[] headers, byte[] response) {
	            	...
	            	try {
	            		...
	            		for (int i = 0; i < dataArray.length(); i++) {
	            			...
	            			demoItem.name = object.getString("name");
	            			demoItem.image = object.getString("image");
	            			demoItem.dep_id = object.getInt("dep_id");
	            			demoItem.dept_name = object.getString("dept_name");
	            			demoItem.price = object.getString("price");
	            			demoItem.desc = object.getString("description");
	            			aryAllDepartList.add(demoItem);
	            			
	            			if (tmpPreviousDepId != object.getInt("dep_id")) {
	            				DepartInfo item = new DepartInfo();
		            			item.id = object.getInt("dep_id");
		            			item.desc = object.getString("dept_name");
		            			item.title = object.getString("dept_name");
		            			aryDepartList.add(item);
	            			}
	            			tmpPreviousDepId = object.getInt("dep_id");
	            		}
